...
class UpdateShoppingItemUseCase
{
    ...
    public function execute(ShoppingItem $item, string $name, ?string $quantity): ShoppingItemData
    {
        return ShoppingItemData::fromModel($this->shopping->updateItem($item, trim($name), $quantity));
    }
}
